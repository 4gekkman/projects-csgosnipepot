...
// Консольная команда //
//--------------------//
class T20_check_escrow_hold_days extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm8:check_escrow_hold_days {id_bot} {partner} {token}';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Check escrow hold in days for specified partner';
  ...
  public function handle()
  {
    ...
    // 1. Выполнить команду
    $result = runcommand('\M8\Commands\C23_check_escrow_hold_days', $this->argument());
    ...
    // 3. В случае успеха, вывести соотв.сообщение
    $this->info("Success");
    $this->info("could_trade: ".(($result['data']['could_trade'] == 1) ? 'yes' : 'no'));
    $this->info("g_daysMyEscrow: ".$result['data']['g_daysMyEscrow']);
    $this->info("g_daysTheirEscrow: ".$result['data']['g_daysTheirEscrow']);
