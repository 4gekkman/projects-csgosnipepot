 ...
 *  Что делает
 *  ----------
 *    - Cancel the specified active trade
 *
...
// Команда //
//---------//
class C11_cancel_the_active_trade extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C11_cancel_the_active_trade from M-package M14 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M14', 'C11_cancel_the_active_trade']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
