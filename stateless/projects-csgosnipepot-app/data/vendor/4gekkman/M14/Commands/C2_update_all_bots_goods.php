 ...
 *  Что делает
 *  ----------
 *    - Updates all shop bot goods in cache
 *
...
// Команда //
//---------//
class C2_update_all_bots_goods extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    $res = call_user_func(function() { try {
      ...
      foreach($bots as $bot) {
        ...
        $errortext = 'Invoking of command C2_update_all_bots_goods from M-package M14 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M14', 'C2_update_all_bots_goods']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
