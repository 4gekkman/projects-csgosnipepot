 ...
 *
 *  s9. Модель виджета выбора языка
 *
 *  	s9.1. Объект-контейнер для всех свойств модели
 *    s9.2. Список поддерживаемых языков
 *    s9.3. Какой язык выбран
 *    s9.n. Индексы и вычисляемые значения
 *
...
var LayoutModelProto = { constructor: function(LayoutModelFunctions) {
	...
	//-------------------------------------------------------------//
		...
		self.m.s0.logged_in_steam_users_declension = ko.computed(function(){
			...
			if(declension == 1) return self.f.s0.localize('peopleonsite:people1');
			if(declension == 2) return self.f.s0.localize('peopleonsite:people2');
			if(declension == 3) return self.f.s0.localize('peopleonsite:people3');
			return self.f.s0.localize('peopleonsite:people3');
	...
	self.m.s1.subdocs = ko.mapping.fromJS([
		{
			...
			icon_url:   '',
			icon_svg:   '',
			title:      self.f.s0.localize('menu:item:cg'),
			bg_color:   '#284351',
			brd_color:  '#2f5463',
			visible:    true,
			vis4anon:   true,
		...
		},
		{
			...
			icon_url:   '',
			icon_svg:   '',
			title:      self.f.s0.localize('menu:item:shop'),
			bg_color:   '#223340',
		...
		},
		{
			...
			title:      self.f.s0.localize('menu:item:profile'),
			bg_color:   '#223340',
			brd_color:  'transparent',
			visible:    true,
			vis4anon:   true,
		...
		},
		{
			...
			title:      self.f.s0.localize('menu:item:top'),
			bg_color:   '#223340',
			brd_color:  'transparent',
			visible:    true,
			vis4anon:   true,
		...
		},
		{
			...
			title:      self.f.s0.localize('menu:item:faq'),
			bg_color:   '#223340',
			brd_color:  'transparent',
			visible:    true,
			vis4anon:   true,
		...
		},
		{
			...
			title:      self.f.s0.localize('menu:item:support'),
			bg_color:   '#223340',
			brd_color:  'transparent',
			visible:    true,
			vis4anon:   true,
		...
		},
		{
			...
			title:      self.f.s0.localize('menu:item:free'),
			bg_color:   '#223340',
			brd_color:  'transparent',
			visible:    true,
			vis4anon:   true,
	...
	ko.computed(function(){
		...
		(function(){
			...
			if(['Lottery', 'Winner', 'Finished'].indexOf(self.m.s6.status.name()) != -1)
				self.m.s6.status.title(self.f.s0.localize('menu:item:cg:lottery') + ":");

			// В противном случае
			else
				self.m.s6.status.title(self.f.s0.localize('menu:item:cg:atstake') + ":");
  ...
  self.m.s8.source = {};
	...
	// s9.2. Список поддерживаемых языков //
	//------------------------------------//
	self.m.s9.langs = ko.mapping.fromJS([
		{
			locale: 'ru',
			flag_bg_pos: '-96px -32px'
		},
		{
			locale: 'en',
			flag_bg_pos: '-64px -64px'
		}
	]);
	...
	// s9.3. Какой язык выбран //
	//-------------------------//
	self.m.s9.choosen_lang = ko.observable((function(){

		return self.m.s9.langs()[0];
	...
	ko.computed(function(){
		...
		//--------------------------------------------------------------//
		// s9.n.1. Объект-контейнер для индексов и вычисляемых значений //
		//--------------------------------------------------------------//
		self.m.s9.indexes = {};
			...
			// 1] Индекс языков //
			//------------------//
			// - По локали языка можно получить ссылку на соотв. объект в self.m.s9.langs
			self.m.s9.indexes.langs = (function(){

				// 1. Подготовить объект для результатов
				var results = {};

				// 2. Заполнить results
				for(var i=0; i<self.m.s9.langs().length; i++) {
					results[self.m.s9.langs()[i].locale()] = self.m.s9.langs()[i];
				}
	...
	ko.computed(function(){
		...
		//-------------------------------------------------//
		// X1.13. Инициализировать интерфейс выбора локали //
		//-------------------------------------------------//
		(function(){

			// 1] Получить ссылку на объект локали
			var locale = self.m.s9.indexes.langs[layout_data.data.locale];
			if(!locale)
				locale = self.m.s9.langs()[0];

			// 2] Выбрать locale
			self.m.s9.choosen_lang(locale);
