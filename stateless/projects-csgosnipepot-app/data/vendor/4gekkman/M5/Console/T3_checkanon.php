 ...
 *  Что делает
 *  ----------
 *    - Check whether an anonymous user in the system
 *
 *  Аргументы
 *  ---------
 *
...
// Консольная команда //
//--------------------//
class T3_checkanon extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:checkanon';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Check whether an anonymous user in the system';
  ...
  public function handle()
  {
    ...
    // 1. Выполнить команду
    $result = runcommand('\M5\Commands\C3_checkanon');
    ...
    // 3. Если анонимный пользователь отсутствует в системе
    if(!$result['data']) $this->info("Anonymous user is absent in the system.");


    // 4. Если анонимный пользователь присутствует в системе
    if($result['data'])
      $this->info("Anonymous user is present in the system: ".$result['data']->name." (id - ".$result['data']->id.', email - '.($result['data']->email ? $result['data']->email : "''").', phone - '.($result['data']->phone ? $result['data']->phone : "''").').');
