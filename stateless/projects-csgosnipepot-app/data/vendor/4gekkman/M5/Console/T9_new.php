 ...
 *  Что делает
 *  ----------
 *    - Create a new user / group / privilege / tag
 *
 *  Аргументы
 *  ---------
 *
...
class T9_new extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:new';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Create a new user / group / privilege / tag';
  ...
  public function handle()
  {
    ...
    //-------------------------------------------------------------//
      ...
      $restype = $this->choice('Choose type of resource that you want to create', $restypes, 'u');
    ...
    $params = call_user_func(function() USE ($restype) { try {
      ...
      if($restype == 'g') {
        ...
        $params['name'] = $this->anticipate("[REQUIRED] Name", []);

        // 3] Запросить необязательные параметры
        $params['description'] = $this->anticipate("[REQUIRED] Description", [], "0");
        $params['isadmin'] = $this->choice("[NOT REQUIRED] Is admin", ["yes" => "Yes, it's group for admins", "no" => "No, it's group not for admins"], 'no');
    ...
    //---------------------------------------------------------------------------//
      ...
      // 3.1. Подготовить список команд
        ...
        $commandsarr = [
          "u"       => "\\M5\\Commands\\C9_newuser",
          "g"       => "\\M5\\Commands\\C10_newgroup",
          "t"       => "\\M5\\Commands\\C12_newtag",
          "cp"      => "\\M5\\Commands\\C11_newprivilege",
        ];
        ...
        // 3.1.2. Выполнить команду для создания ресурса типа $restype
          ...
          // 3] В случае успеха
          switch($restype) {
            case "u"      : $this->info("New user with id = '".$result['data']['id']."' has been successfully created."); break;
            case "g"      : $this->info("New group '".$result['data']['name']."' has been successfully created."); break;
            case "t"      : $this->info("New tag '".$result['data']['name']."' has been successfully created."); break;
            case "cp"     : $this->info("New custom privilege '".$result['data']['name']."' has been successfully created."); break;
