 ...
 *  Что делает
 *  ----------
 *    - Restore a user / group / privilege / tag
 *
 *  Аргументы
 *  ---------
 *
...
// Консольная команда //
//--------------------//
class T14_restore extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:restore';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Restore a user / group / privilege / tag';
  ...
  public function handle()
  {
     ...
     * Оглавление
     *
     *  1. Выяснить тип ресурса, который пользователь хочет восстановить
     *  2. Запросить у пользователя соответствующие восстанавливаемому типу ресурса параметры
     *  3. Выполнить команду для восстановления желаемого ресурса, передав ей параметры
     *
     */
    ...
    //------------------------------------------------------------------//
      ...
      $restypes = [
        ...
        "g"       => "Restore group",
        "t"       => "Restore tag",
        "cp"      => "Restore custom privilege",
      ];

      // 1.2. Узнать у пользователя, ресурс какого типа он хочет создать
      $restype = $this->choice('Choose type of resource that you want to restore', $restypes, 'u');
    ...
    $params = call_user_func(function() USE ($restype) { try {
      ...
      if($restype == 'cp') {
        ...
        // 2] Запросить обязательные параметры
        $params['id'] = $this->anticipate("[REQUIRED] ID of the privilege that you want to restore", []);
    ...
    //---------------------------------------------------------------------------------//
      ...
      // 3.1. Подготовить список команд
        ...
        $commandsarr = [
          "u"       => "\\M5\\Commands\\C27_restoreuser",
          "g"       => "\\M5\\Commands\\C28_restoregroup",
          "t"       => "\\M5\\Commands\\C30_restoretag",
          "cp"      => "\\M5\\Commands\\C29_restoreprivilege",
        ];
        ...
        // 3.1.2. Выполнить команду для создания ресурса типа $restype
          ...
          // 3] В случае успеха
          switch($restype) {
            case "u"      : $this->info("User ".$result['data']['name']." (".$result['data']['email'].") has been restored successfully."); break;
            case "g"      : $this->info("Group '".$result['data']['name']."' has been restored successfully."); break;
            case "t"      : $this->info("Tag '".$result['data']['name']."' has been restored successfully."); break;
            case "cp"     : $this->info("Custom privilege '".$result['data']['name']."' has been restored successfully."); break;
