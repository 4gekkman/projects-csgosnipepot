 ...
 *  Что делает
 *  ----------
 *    - Calls c.command m5:update, which invokes update of DB of M5
 *
...
////===================================================////
class H1_update  // TODO: написать "implements ShouldQueue", и тогда обработчик будет добавляться в очередь задач, а не исполняться в реалтайм
{
   ...
   *
   *  1. Выполнить к.команду m5:update
   *
   *  X. Вернуть результат
   *
   */
    ...
    try {
      ...
      $errortext = 'Keys checking in event handler H1_update of M-package M5 have ended with error: '.$e->getMessage();
      Log::info($errortext);
      write2log($errortext, ['M5', 'H1_update']);
      return [
        "status"  => -2,
        "data"    => $errortext
      ];
    ...
    $res = call_user_func(function() USE ($event) { try {
      ...
      return runcommand('\M5\Commands\C1_update', $event->data);

    } catch(\Exception $e) {
        DB::rollback();
        $errortext = 'Invoking of event handler H1_update of M-package M5 have ended with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M5', 'H1_update']);
        return [
          "status"  => -2,
