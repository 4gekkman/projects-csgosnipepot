...
// Команда //
//---------//
class C12_offers_toothcomb extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    $res = call_user_func(function() { try {
      ...
      // a. Сделать запись о том, что команда начала выполняться
        ...
        $executing = json_decode(Cache::get('m13:processing:offers_toothcomb_executing'), true);
        ...
        Cache::put('m13:processing:offers_toothcomb_executing', json_encode($executing, JSON_UNESCAPED_UNICODE), 300);
      ...
      // 5. Записать $trades_ex_active в кэш
      Cache::put('m13:processing:trades_ex_active', json_encode($trades_ex_active, JSON_UNESCAPED_UNICODE), 60);
      ...
      // Сделать запись о том, что команда закончила выполняться
        ...
        // a.3. Записать $executing в кэш
        Cache::put('m13:processing:offers_toothcomb_executing', json_encode($executing, JSON_PRETTY_PRINT), 300);

      // Остальное

        $errortext = 'Invoking of command C12_offers_toothcomb from M-package M13 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M13', 'C12_offers_toothcomb']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
