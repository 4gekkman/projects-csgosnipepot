...
////===================================================////
class H2_ticks  // TODO: написать "implements ShouldQueue", и тогда обработчик будет добавляться в очередь задач, а не исполняться в реалтайм
{
   ...
   *
   *  1. Процессинг магазина, производить не чаще, чем раз в 10 секунд
   *
   *  X. Вернуть результат
   *
   */
    ...
    try {
      ...
      $errortext = 'Keys checking in event handler H2_ticks of M-package M14 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
      Log::info($errortext);
      write2log($errortext, ['M14', 'H2_ticks']);
      return [
        "status"  => -2,
        "data"    => $errortext
      ];
    ...
    $res = call_user_func(function() USE ($event) { try {
      ...
      call_user_func(function(){
        ...
        $cache = Cache::get('m14:processing:last_datetime');
        if(empty($cache))
          $cache = 0;
        ...
        Cache::put('m14:processing:last_datetime', $cache, 30);

        // 1.4. Если $cache >= 10 секундам, запустить C5_processor
        if($cache >= 10) {

          // 1] Запустить процессор
          $result = runcommand('\M14\Commands\C6_processor', [], 0, ['on'=>true, 'name'=>'m14_processor']);
          if($result['status'] != 0)
            throw new \Exception($result['data']['errormsg']);

          // 2] Сбросить кэш в 0
          Cache::put('m14:processing:last_datetime', 0, 30);
        ...
        $errortext = 'Invoking of event handler H2_ticks of M-package M14 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M14', 'H2_ticks']);
        return [
          "status"  => -2,
