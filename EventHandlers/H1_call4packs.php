 ...
 *  Что делает
 *  ----------
 *    - Catches event from M4, starts m1:parseapp, which sends data to M4 to update its DB
 *
...
////===================================================////
class H1_call4packs  // TODO: написать "implements ShouldQueue", и тогда обработчик будет добавляться в очередь задач, а не исполняться в реалтайм
{
   ...
   */
    ...
    try {
      ...
      $errortext = 'Keys checking in event handler H1_call4packs of M-package M1 have ended with error: '.$e->getMessage();
      Log::info($errortext);
      write2log($errortext, ['M1', 'H1_call4packs']);
      return [
        "status"  => -2,
        "data"    => $errortext
      ];
    ...
    $res = call_user_func(function() USE ($event) { try {
      ...
      if($result['status'] != 0)
        ...
        DB::rollback();
        $errortext = 'Invoking of event handler H1_call4packs of M-package M1 have ended with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M1', 'H1_call4packs']);
        return [
          "status"  => -2,
