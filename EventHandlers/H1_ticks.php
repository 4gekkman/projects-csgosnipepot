   ...
   */
    ...
    $res = call_user_func(function() USE ($event) { try {
      ...
      (call_user_func(function(){
        ...
        // 5] Если разница между $last и $cur более 15 секунд
        // - Перезапустить все queue worker-ы.
        // - Записать в кэш текущие дату/время, чтобы оно не перезапускалось ежесекундно, а лишь раз в 15 сек.
        if($cur->diffInSeconds($last) > 15 && Cache::get('m9:processing:restarting') != '1') {
          ...
          // 5.4] Перезапустить тики
          Artisan::call('m11:stop');
          sleep(3);
          Artisan::call('m11:start');
          sleep(3);

          // 5.5] Отметить в кэше, что процесс перезапуска закончился
