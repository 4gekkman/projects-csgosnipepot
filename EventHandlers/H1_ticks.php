...
class H1_ticks  // TODO: написать "implements ShouldQueue", и тогда обработчик будет добавляться в очередь задач, а не исполняться в реалтайм
{
   ...
   */
    ...
    try {
      ...
      $errortext = 'Keys checking in event handler H1_ticks of M-package M16 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
      Log::info($errortext);
      write2log($errortext, ['M16', 'H1_ticks']);
      return [
        "status"  => -2,
        "data"    => $errortext
      ];
    ...
    $res = call_user_func(function() USE ($event) { try { DB::beginTransaction();
      ...
      // 1. Процессинг системы "Будь онлайн"
      $result = runcommand('\M16\Commands\C1_processor', [], 0, ['on'=>true, 'name'=>'m16_processor']);
      if($result['status'] != 0)
    ...
    DB::commit(); } catch(\Exception $e) {
        DB::rollback();
        $errortext = 'Invoking of event handler H1_ticks of M-package M16 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M16', 'H1_ticks']);
        return [
          "status"  => -2,
