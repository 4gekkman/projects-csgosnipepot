...
class H1_update  // TODO: написать "implements ShouldQueue", и тогда обработчик будет добавляться в очередь задач, а не исполняться в реалтайм
{
   ...
   */
    ...
    try {
      ...
      $errortext = 'Keys checking in event handler H1_update of M-package M4 have ended with error: '.$e->getMessage();
      Log::info($errortext);
      write2log($errortext, ['M4', 'H1_update']);
      return [
        "status"  => -2,
        "data"    => $errortext
      ];
