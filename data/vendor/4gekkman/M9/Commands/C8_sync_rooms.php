...
class C8_sync_rooms extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  2. Добавить/Восстановить-обновить в БД те комнаты, которые есть в $rooms
     *
     *  N. Вернуть статус 0
     *
     */
    ...
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      // 2. Создать в БД комнаты из $rooms
      foreach($rooms as $name => $room) {

        // 1] Создать новую комнату
        $newroom = runcommand('\M9\Commands\C2_create_new_room', [
          "name" => $room['name']
        ]);
        if($newroom['status'] != 0)
          throw new \Exception($newroom['data']['errormsg']);

        // 2] Обновить некоторые свойства $newroom
        $newroom['data']['room']->is_on             = $room['is_on'];
        $newroom['data']['room']->description       = $room['description'];
        $newroom['data']['room']->description_full  = $room['description_full'];
        $newroom['data']['room']->save();
