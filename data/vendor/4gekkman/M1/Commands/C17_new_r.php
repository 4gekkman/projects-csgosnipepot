  ...
  public function handle()
  {
    ...
    $res = call_user_func(function() { try {
      ...
      $packid = call_user_func(function() USE ($packid) {
        ...
        // 3.3. Отфильтровать из $all_github_user_packs['result'] все не R-пакеты
        $all_github_user_packs['result'] = array_values(collect($all_github_user_packs['result'])->filter(function($item){
          if(!preg_match("/^[R]{1}[1-9]{1}[0-9]*$/ui", $item))
            return false;
          return true;
        })->toArray());
        ...
        $all_github_user_packs_nums = collect($all_github_user_packs['result'])->map(function($item){
          return preg_replace("/^R/ui", "", $item);
        })->toArray();

        // 3.5. Получить список ID (номеров) всех R-пакетов, которые фактически есть в проекте
        ...
        if(!empty($packid)) {
          if(in_array($packid, $packids)) {
            throw new \Exception("Can't create R-package with id $packid, because R-package with id $packid already exists.");
          }
        }
