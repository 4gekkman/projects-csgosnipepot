...
class H1_ticks  // TODO: написать "implements ShouldQueue", и тогда обработчик будет добавляться в очередь задач, а не исполняться в реалтайм
{
   ...
   */
    ...
    try {
      ...
      $errortext = 'Keys checking in event handler H1_ticks of M-package M1 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
      Log::info($errortext);
      write2log($errortext, ['M1', 'H1_ticks']);
      return [
        "status"  => -2,
        "data"    => $errortext
      ];
    ...
    $res = call_user_func(function() USE ($event) { try {
      ...
      // TODO: текст обработчика
        ...
        $errortext = 'Invoking of event handler H1_ticks of M-package M1 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M1', 'H1_ticks']);
        return [
          "status"  => -2,
