 ...
 *  Что делает
 *  ----------
 *    - Shows the table of D,L,W-packs and their routes
 *
 *  Аргументы
 *  ---------
 *
...
class T2_list extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm4:list';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Shows the table of D,L,W-packs and their routes';
  ...
  public function handle()
  {
    ...
    // 1. Выполнить команду, в случае неудачи, вывести текст ошибки
    ...
    // 2. Вывести информацию о роутах в терминал
    $this->table(['ID', 'type', 'ison', 'package', 'protocol', 'subdomain', 'domain', 'uri'], json_decode($result['data']['prepeared'], true));

    // 3. Вывести информацию о коллизиях в терминал

      // 3.1. Выполнить команду, в случае неудачи, вывести текст ошибки
      $result = runcommand('\M4\Commands\C3_check');
      if($result['status'] != 0) {
        $this->error('Error: '.$result['data']);
        return;
      }

      // 3.2. Вывести информацию о коллизиях в терминал

        // Если коллизий нет
        if(count(json_decode($result['data'],true)) == 0)
          $this->info('There is no collisions');

        // Если коллизии есть
        else {
          $this->error('There is following collisions');
          $this->table(['route_id_1', 'route_pack_1', 'route_id_2', 'route_pack_2', 'domain', 'protocol', 'subdomain', 'uri'], json_decode($result['data'],true));
        }
