...
// Команда //
//---------//
class C2_list extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    //--------------------------------------------------------//
    // Для всех D,L,W-пакетов получить все роуты и их свойтва //
    //--------------------------------------------------------//
    $res = call_user_func(function() { try {
      ...
      return [
        ...
        "data"    => [
          ...
          "prepeared"   => json_encode($prepeared_data, JSON_UNESCAPED_UNICODE),
        ]
      ];

    } catch(\Exception $e) {
        $errortext = 'Invoking of command C2_list from M-package M4 have ended with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M4', 'C2_list']);
        return [
          "status"  => -2,
          "data"    => $errortext
        ];
