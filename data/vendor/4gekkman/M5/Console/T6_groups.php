...
// Консольная команда //
//--------------------//
class T6_groups extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:groups';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Get groups list (can use filters)';
  ...
  public function handle()
  {
    ...
    $functions_list = [
      ...
      "details"     => ["description" => "Get info about specified group in details", "src" => function() USE (&$options, &$exit){

        // 1] Спросить ID группы
        while(!is_numeric($id = $this->anticipate("Enter the user ID", []))) {}

        // 2] Получить группу с $id
        $group = \M5\Models\MD2_groups::find($id);
        if(empty($group)) {
          $this->error('Группа с ID = '.$id.' не найдена (среди активных, т.е. не мягко удалённых)');
          return;
        }

        // 3] Лениво подгрузить связи группы
        $group->load(['tags', 'privileges']);
      ...
      "filters_add_name"      => ["description" => "Add a new value to the filter by names", "src" => function() USE (&$options){
        ...
        // 2] Запросить имя
        $name = $this->anticipate("Add name (enter group name)", []);
    ...
    while($exit != "yes") {
      ...
      $result = runcommand('\M5\Commands\C6_groups', $options);
      ...
      // 4.4. Вывести список групп на текущей странице в терминал
      $this->table(call_user_func(function() {
        $result = ['id', 'name', 'description', 'created_at'];
        return $result;
      }), $result['data']['groups']->transform(function($item, $key) {
        return [$item->id, $item->name, $item->description, $item->created_at];
      }));
