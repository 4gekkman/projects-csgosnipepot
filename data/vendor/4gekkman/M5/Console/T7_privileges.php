...
// Консольная команда //
//--------------------//
class T7_privileges extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:privileges';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Get get privileges list (can use filters)';
  ...
  public function handle()
  {
    ...
    $functions_list = [
      ...
      "details"     => ["description" => "Get info about specified privilege in details", "src" => function() USE (&$options, &$exit){

        // 1] Спросить ID права
        while(!is_numeric($id = $this->anticipate("Enter the privilege ID", []))) {}

        // 2] Получить право с $id
        $group = \M5\Models\MD3_privileges::find($id);
        if(empty($group)) {
          $this->error('Право с ID = '.$id.' не найдено (среди активных, т.е. не мягко удалённых)');
          return;
        }

        // 3] Лениво подгрузить связи права
        $group->load(['tags', 'groups']);

        // 4] Сформировать красивую строку для вывода
        $output = json_encode($group, JSON_PRETTY_PRINT);
      ...
      "filters"            => ["description" => "Reset and update all filters", "src" => function() USE (&$options){
        ...
        if($need_filters == 'yes') {
          ...
          // 2.1] ids
          $options['filters']['ids'] = $this->anticipate("By ids (specify group ids separated by commas, or leave blank)", [], "next");
          $options['filters']['ids'] = $options['filters']['ids'] == "next" ? [] : explode(",", $options['filters']['ids']);

          // 2.2] names
          $options['filters']['names'] = $this->anticipate("By names (specify group names separated by commas, or leave blank)", [], "___next___");
          $options['filters']['names'] = $options['filters']['names'] == "___next___" ? [] : explode(",", $options['filters']['names']);

          // 2.3] users
          $options['filters']['users'] = $this->anticipate("By users (specify user ids separated by commas, or leave blank)", [], "next");
          $options['filters']['users'] = $options['filters']['users'] == "next" ? [] : explode(",", $options['filters']['users']);
      ...
      "filters_add_name"      => ["description" => "Add a new value to the filter by names", "src" => function() USE (&$options){

        // 1] Показать текущий состав фильтра
        $this->info(implode(',', $options['filters']['names']));

        // 2] Запросить имя
        $name = $this->anticipate("Add name (enter privilege name)", []);

        // 3] Добавить указанное значение
        array_push($options['filters']['names'], $name);

      }],

      "filters_add_user"      => ["description" => "Add a new value to the filter by users", "src" => function() USE (&$options){

        // 1] Показать текущий состав фильтра
        $this->info(implode(',', $options['filters']['users']));

        // 2] Запросить ID группы
        $user = $this->anticipate("Add user (enter user id)", []);

        // 3] Добавить указанное значение
        array_push($options['filters']['users'], $user);
    ...
    while($exit != "yes") {
      ...
      $result = runcommand('\M5\Commands\C7_privileges', $options);
      ...
      // 4.4. Вывести список прав на текущей странице в терминал
      $this->table(call_user_func(function() {
        $result = ['id', 'type', 'name', 'description', 'created_at'];
        return $result;
      }), $result['data']['privileges']->transform(function($item, $key) {
        return [$item->id, $item->privtypes->name, $item->name, $item->description, $item->created_at];
      }));
