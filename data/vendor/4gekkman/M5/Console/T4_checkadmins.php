...
// Консольная команда //
//--------------------//
class T4_checkadmins extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:checkadmins';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Check whether admins in the system';
  ...
  public function handle()
  {
    ...
    // 1. Выполнить команду
    $result = runcommand('\M5\Commands\C4_checkadmins');
    ...
    // 3. Если админы отсутствуют в системе
    if(!$result['data']) $this->info("Administrators are absent in the system.");


    // 4. Если админы присутствуют в системе
    if($result['data'] && count($result['data']) != 0) {
      $this->table(['id','name','email','phone'], $result['data']->transform(function($item, $key){ return [$item->id, $item->name, $item->email ?: "''", $item->phone ?: "''"]; }));
    }
