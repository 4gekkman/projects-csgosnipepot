 ...
 *  Что делает
 *  ----------
 *    - Get all user privileges, taking into account personal and privileges of groups, where user consists
 *
 *  Аргументы
 *  ---------
 *
...
// Консольная команда //
//--------------------//
class T15_getuserprivs extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:getuserprivs';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Get all user privileges, taking into account personal and privileges of groups, where user consists';
  ...
  public function handle()
  {
     ...
     * Оглавление
     *
     *  1. Спросить, для какого пользователя требуется получить права
     *  2. Выполнить команду
     *  3. В случае неудачи, вывести текст ошибки
     *  4. Подготовить данные для вывода в таблице в терминале
     *  5. Вывести результаты в терминал
     *
     */

    // 1. Спросить, для какого пользователя требуется получить права
    $id_user = $this->anticipate("[REQUIRED] ID of the user for which you want to get list of privileges", []);

    // 2. Выполнить команду
    $result = runcommand('\M5\Commands\C31_getuserprivs', ['id' => $id_user]);
    ...
    // - Отсортировать по type
      ...
      // 4.1. Подготовить
      $output4table = $result['data']->transform(function($item, $key){ return [$item->id, $item->privtypes->name, $item->name, $item->description]; })->toArray();

      // 4.2. Отсортировать
      $weights = ['exec' => 0, 'access' => 1, 'custom' => 2];
      usort($output4table, function($a, $b) USE ($weights) {
        $a = array_key_exists($a[1], $weights) ? $weights[$a[1]] : 0;
        $b = array_key_exists($b[1], $weights) ? $weights[$b[1]] : 0;
        if ($a == $b) return 0;
        return ($a < $b) ? -1 : 1;
      });

    // 5. Вывести результаты в терминал
    $this->table(['id', 'type', 'name', 'description'], $output4table);
