...
// Команда //
//---------//
class C21_attachuser extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  4. Если $user ещё нет в $group, добавить
     *  5. Сделать commit
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C21_attachuser from M-package M5 have ended with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M5', 'C21_attachuser']);
        return [
          "status"  => -2,
          "data"    => $errortext
        ];
