 ...
 *  Что делает
 *  ----------
 *    - Detach a tag from a user / group / privilege
 *
...
// Команда //
//---------//
class C26_detachtag extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  6. Если $user/$group/$privilege имеет такой $tag, изъять
     *  7. Сделать commit
    ...
    //-------------------------------------------------------//
    // Изъять тег у указанного пользователя / группы / права //
    //-------------------------------------------------------//
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      // 6. Если $user/$group/$privilege имеет такой $tag, изъять
        ...
        // 6.1. Если тег надо изъять у пользователя
        if(array_key_exists('id_user', $this->data)) {
          if($user->tags->contains($tag->id))
            $user->tags()->detach($tag->id);
        }

        // 6.2. Если тег надо изъять у группы
        if(array_key_exists('id_group', $this->data)) {
          if($group->tags->contains($tag->id))
            $group->tags()->detach($tag->id);
        }

        // 6.3. Если тег надо иззъять у права
        if(array_key_exists('id_privilege', $this->data)) {
          if($privilege->tags->contains($tag->id))
            $privilege->tags()->detach($tag->id);
        }
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C26_detachtag from M-package M5 have ended with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M5', 'C26_detachtag']);
        return [
          "status"  => -2,
          "data"    => $errortext
        ];
