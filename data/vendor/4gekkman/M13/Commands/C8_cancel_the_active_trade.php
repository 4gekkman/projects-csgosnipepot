...
// Команда //
//---------//
class C8_cancel_the_active_trade extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C8_cancel_the_active_trade from M-package M13 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M13', 'C8_cancel_the_active_trade']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
