...
// Команда //
//---------//
class C13_active_offers_tracking extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  5. Получить из кэша информацию о состоянии активных офферов из m14.C15
    ...
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      //    ]
      //
      $executing = json_decode(Cache::get('m14:processing:trades_ex_active'), true);
      ...
      call_user_func(function() USE ($trades_ex_active) {
        for($i=0; $i<count($trades_ex_active); $i++) {
          ...
          if($id_status_new == 3) {
            ...
            $result = runcommand('\M14\Commands\C14_active_to_accepted', [
              "tradeid"           => $tradeid,
          ...
          // 3] Если статус оффера изменился НЕ на Accepted
            ...
            $result = runcommand('\M14\Commands\C12_cancel_the_active_trade_dbpart', [
              "tradeid"           => $tradeid,
              "tradeofferid"      => $tradeofferid,
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C13_active_offers_tracking from M-package M14 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M14', 'C13_active_offers_tracking']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
