 ...
 *  Что делает
 *  ----------
 *    - Check escrow hold in days via tradeofferid
 *
 ...
 *        tradeofferid  | ID активного оффера
 *      ]
 *    ]
 *
...
// Команда //
//---------//
class C35_check_escrow_hold_days_by_tradeofferid extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    $res = call_user_func(function() { try {
      ...
      // 3. Осуществить GET-запрос к steam от имени $bot, и получить HTML-документ в ответ
        ...
        $response = call_user_func(function() USE ($bot) {
          ...
          $result = runcommand('\M8\Commands\C6_bot_request_steam', [
            "id_bot"          => $bot->id,
            "method"          => "GET",
            "url"             => 'https://steamcommunity.com/tradeoffer/'.$this->data['tradeofferid'],
      ...
      return [
        ...
        $errortext = 'Invoking of command C35_check_escrow_hold_days_by_tradeofferid from M-package M8 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M8', 'C35_check_escrow_hold_days_by_tradeofferid']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
