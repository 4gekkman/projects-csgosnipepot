...
// Команда //
//---------//
class C27_del_w extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  2. Проверить существование W-пакета $packid
     *  3. Отменить автосохранение пакета $packid на github
     *  4. Удалить каталог W-пакета $packid
     *  5. Удалить пр.имён W-пакета из composer.json проекта -> autoload -> psr-4
     *  6. Удалить файл-конфиг W-пакета (если требуется)
    ...
    $res = call_user_func(function() { try {
      ...
      return [
        ...
        $errortext = 'Invoking of command C27_del_w from M-package M1 have ended with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M1', 'del_w']);
        return [
          "status"  => -2,
          "data"    => $errortext
        ];
