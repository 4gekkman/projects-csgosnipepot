...
// Команда //
//---------//
class C27_cancel_trade_offer extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    //----------------------------------------------------------------------------------//
    // Отменить отправленное ранее указанным ботом указанное торговое предложение по ID //
    //----------------------------------------------------------------------------------//
    $res = call_user_func(function() { try {
      ...
      return [
        ...
        $errortext = 'Invoking of command C27_cancel_trade_offer from M-package M8 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M8', 'C27_cancel_trade_offer']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
