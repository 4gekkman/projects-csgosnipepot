 ...
 *
 *  С первого раза может не сработать
 *  ---------------------------------
 *    - Может понадобиться применить несколько раз.
 *
 *
 */
...
// Команда //
//---------//
class C28_accept_trade_offer extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    //---------------------------------------------------//
    // Принять присланное торговое предложение по его ID //
    //---------------------------------------------------//
    $res = call_user_func(function() { try {
      ...
      return [
        ...
        $errortext = 'Invoking of command C28_accept_trade_offer from M-package M8 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        Log::info($errortext);
        write2log($errortext, ['M8', 'C28_accept_trade_offer']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
