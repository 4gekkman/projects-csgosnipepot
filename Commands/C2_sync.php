 ...
 *  Что делает
 *  ----------
 *    - Sync bots with users and vice versa
 *
...
// Команда //
//---------//
class C2_sync extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     * Оглавление
     *
     *  1. Получить названия групп для steam-польз-ей и ботов из конфига
    ...
    //-----------------------------------------------------//
    // Синхронизировать ботов с пользователями, и наоборот //
    //-----------------------------------------------------//
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      foreach($users as $user) {
        ...
        // 3.1. Если связь m8_bots не существует, завершить
        if(!r1_rel_exists("m5", "md1_users", "m8_bots")) break;

        // 3.2. Попробовать найти ботскую информацию для $user
        $bot = $user->m8_bots;

        // 3.3. Если $bot пуст, создать новую строку в m8.md1_bots, и связать
        if($bot->count() == 0) {
          $bot = new \M8\Models\MD1_bots();
          $bot->save();
          if(!$user->m8_bots->contains($bot->id))
            $user->m8_bots()->attach($bot->id);
        }
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C2_sync from M-package M8 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M8', 'C2_sync']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
