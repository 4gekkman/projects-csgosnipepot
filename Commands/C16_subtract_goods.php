 ...
 *  Что делает
 *  ----------
 *    - Subtract sold goods from bots goods cache
 *
...
// Команда //
//---------//
class C16_subtract_goods extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  5. Отфильтровать из $goods все связанные с $trade вещи
    ...
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      // 5. Отфильтровать из $goods все связанные с $trade вещи, добавляя их в корзину в кэше
      $goods = collect($goods)->filter(function($value, $key) USE ($trade_assetid_arr) {

        // 5.1. Если вещь надо отфильтровать
        if(in_array($value['assetid'], $trade_assetid_arr)) {

          // 1] Получить из кэша корзину
          $cart = json_decode(Cache::get("M14:subtracted_goods:cart"), true);
          if(empty($cart))
            $cart = [];

          // 2] Добавить $value в корзину
          array_push($cart, $value);

          // 3] Записать $cart в кэш на час
          Cache::put("M14:subtracted_goods:cart", json_encode($cart, JSON_UNESCAPED_UNICODE), 60);

          // n] Вернуть false (отфильтровав вещь)
          return false;

        }

        // 5.2. Если вещь надо вернуть
        else
          return true;
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C16_subtract_goods from M-package M14 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M14', 'C16_subtract_goods']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
