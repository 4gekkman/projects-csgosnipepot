...
// Команда //
//---------//
class C1_now extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    $res = call_user_func(function() { try {
      ...
      $utcoffset = call_user_func(function() USE ($utcoffset){
        if(+$utcoffset > 0) return '+'.(int)$utcoffset;
        else if(+$utcoffset < 0) return (int)$utcoffset;
        else { return '0'; }
      });

      // 6. Создать новый объект Carbon с UTC-смещением $utcoffset
      if(!empty($utcoffset)) $carbon = \Carbon\Carbon::now($utcoffset);
      else $carbon = \Carbon\Carbon::now();
