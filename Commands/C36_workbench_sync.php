...
class C36_workbench_sync extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      // 2. Проверить существование базы данных пакета $this->data['data']['packid']
      // - Если не существует, сообщить и завершить без ошибки
      if(count(DB::select("SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '".mb_strtolower($this->data['data']['packid'])."'")) == 0) {
        write2log('База данных пакета '.$this->data['data']['packid'].' не найдена.');
        return [
          "status"  => 0,
          "data"    => ""
        ];
      ...
      // 6. Создать модели из списка $list_final для пакета $this->data['data']['packid']
        ...
        // 6.2. Подождать, пока все файлы из $list_final не будут существовать
        // - И только затем продолжить

          // 1] Написать функцию, проверяющую существование файлов из $list_final
          $check_models_exist = function() USE ($list_final) {
            foreach($list_final as $model) {
              if(!file_exists(base_path('vendor/4gekkman/'.$this->data['data']['packid'].'/Models/'.preg_replace("/^md/ui","MD",$model['table']).'.php')))
                return false;
            }
            return true;
          };

          // 2] Ждать, пока все файлы не будут созданы
          while(!$check_models_exist()) {
            usleep(100000);
          }
      ...
      foreach($relationships2add[$this->data['data']['packid']] as $model => $rels) {
        ...
        $file = $this->storage->get(preg_replace("/^md/ui","MD",$model).'.php');
        ...
        config(['filesystems.disks.local.root' => base_path('vendor/4gekkman/'.$this->data['data']['packid'].'/Models')]);
        $this->storage = new \Illuminate\Filesystem\FilesystemManager(app());
        $this->storage->put(preg_replace("/^md/ui","MD",$model).'.php', $file);

      }


    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C36_workbench_sync from M-package M1 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        return [
          "status"  => -2,
