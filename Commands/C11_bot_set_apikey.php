...
class C11_bot_set_apikey extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  4. Если поле apikey у $bot пустое, или если force = 1, обновить API-ключ $bot'а
     *
     *  N. Вернуть статус 0
     *
     */
    ...
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      if(empty($bot->apikey) || $this->data['force'] == 1) {
        ...
        // 4.1. Подготовить функцию для извлечения API-ключа пользователя
        // - Если надо, она также регистрирует пользователю новый ключ.
        $bot_get_apikey = function($recursionLevel = 1) USE ($bot, $is_bot_authorized) {

          // 4.1.1. Запросить от имени бота HTML страницы с API-ключём
          ...
          // 4.1.2. Извлечь строку с HTML из запроса
          $apikey_html = (string) $apikey_html_response->getBody();

          // 4.1.3. Если по какой-то причине доступ боту ограничен, apikey = ''
          if(preg_match('/<h2>Access Denied<\/h2>/', $apikey_html)) {
            $bot->apiKey = '';
            $bot->save();
          }

          // 4.1.4. Если доступ есть, и API-ключ получен
          else if(preg_match('/<p>Key: (.*)<\/p>/', $apikey_html, $matches)) {
            $bot->apiKey = $matches[1];
            $bot->save();
          }

          // 4.1.5. Если доступ есть, но API-ключ не получен
