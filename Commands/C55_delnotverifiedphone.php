 ...
 *  Что делает
 *  ----------
 *    - Every hour check and del users with not verified phones if their time is expired
 *
...
// Команда //
//---------//
class C55_delnotverifiedphone extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    //----------------------------------------------------------------------------------------------------------------------------//
    // Каждый час проверять и удалять пользователей с невериф.phone, если оно включено, и они существ.дольше указанного в конфиге //
    //----------------------------------------------------------------------------------------------------------------------------//
    $res = call_user_func(function() { try { DB::beginTransaction();

      // 1. Получить значения параметров из конфиге
      $on = config('M5.del_users_with_not_ver_phones') ?: false;
      $hours = config('M5.del_users_with_not_ver_phones_in_hours') ?: 24;
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C55_delnotverifiedphone from M-package M5 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M5', 'C55_delnotverifiedphone']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
