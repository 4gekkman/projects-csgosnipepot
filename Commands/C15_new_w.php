...
// Команда //
//---------//
class C15_new_w extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  2. Определить $packid
     *  3. Сформировать внутренний ID пакета (напр.: W1)
     *  4. Скопировать и переименовать каталог W
     *  5. Заменить плейсхолдеры в файлах нового W-пакета
     *  6. Добавить пр.имён W-пакета в composer.json проекта -> autoload -> psr-4
    ...
    $res = call_user_func(function() { try {
      ...
      // 1. Провести валидацию и извлечение входящих параметров
        ...
        $name         = $this->data['name'] ?: "";
        $description  = $this->data['description'] ?: "Описание нового W-пакета";
      ...
      $packid = call_user_func(function() USE ($packid) {
        ...
        // 2.3. Отфильтровать из $all_github_user_packs['result'] все не W-пакеты
        $all_github_user_packs['result'] = array_values(collect($all_github_user_packs['result'])->filter(function($item){
          if(!preg_match("/^[W]{1}[1-9]{1}[0-9]*$/ui", $item))
            return false;
          return true;
        })->toArray());
        ...
        $all_github_user_packs_nums = collect($all_github_user_packs['result'])->map(function($item){
          return preg_replace("/^w/ui", "", $item);
        })->toArray();

        // 2.5. Получить список ID (номеров) всех W-пакетов, которые фактически есть в проекте
        ...
        if(!empty($packid)) {
          if(in_array($packid, $packids)) {
            throw new \Exception("Can't create W-package with id $packid, because W-package with id $packid already exists.");
          }
        }
      ...
      $this->storage->copyDirectory('vendor/4gekkman/M1/Samples/W', 'vendor/4gekkman/'.$packfullname);
      ...
      return [
        ...
        $errortext = 'Invoking of command C15_new_w from M-package M1 have ended with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M1', 'new_w']);
        return [
          "status"  => -2,
          "data"    => $errortext
        ];
