...
// Команда //
//---------//
class C8_tags extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
    ...
    //----------------------------------------------------------//
    // Вернуть информацию о тегах с учётом фильтров и пагинации //
    //----------------------------------------------------------//
    $res = call_user_func(function() { try {
      ...
      if(!empty($filters)) {
        $validator = r4_validate($filters, [
          ...
          "2.value"               => ["r4_defined", "regex:/^([1-9]+[0-9]*(,[1-9]+[0-9]*)*|)$/ui"],   // IDs of users
          "3.value"               => ["r4_defined", "regex:/^([1-9]+[0-9]*(,[1-9]+[0-9]*)*|)$/ui"],   // IDs of groups
      ...
      // 4. Сформировать запрос с учётом фильтров, извлечь данные
        ...
        if(!empty($filters)) {
          ...
          if($filters[0]['on'] === true) {
            $ids_of_tags = explode(',', $filters[0]['value']);
          ...
          if($filters[2]['on'] === true) {
            $ids_of_users = explode(',', $filters[2]['value']);
          ...
          if($filters[3]['on'] === true) {
            $ids_of_groups = explode(',', $filters[3]['value']);
      ...
      return [
        ...
        $errortext = 'Invoking of command C8_tags from M-package M5 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M5', 'C8_tags']);
        return [
          "status"  => -2,
          "data"    => $errortext
        ];
