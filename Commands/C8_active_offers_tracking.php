...
// Команда //
//---------//
class C8_active_offers_tracking extends Job { // TODO: добавить "implements ShouldQueue" - и команда будет добавляться в очередь задач
  ...
  public function handle()
  {
     ...
     *  8. Если $trades_ex_active не пуст, обновить кэш активных офферов
     *
     *  N. Вернуть статус 0
     *
     */
    ...
    $res = call_user_func(function() { try { DB::beginTransaction();
      ...
      // 1. Получить активные выдачи из кэша
      ...
      //    ]
      //
      $executing = json_decode(Cache::get('m16:processing:trades_ex_active'), true);
      ...
      call_user_func(function() USE ($trades_ex_active) {
        for($i=0; $i<count($trades_ex_active); $i++) {
          ...
          // 1] Получить выдачу с ID $trades_ex_active[$i]['tradeofferid']
          $giveaway = \M16\Models\MD2_giveaways::where('tradeofferid', $trades_ex_active[$i]['tradeofferid'])
              ->first();

          // 2] Если выдача не найдена, перети к следующей итерации
          if(empty($giveaway))
            continue;

          // 3] Изменить статус выдачи на 3
          $giveaway->giveaway_status = 3;
          $giveaway->save();
      ...
      // 8. Если $trades_ex_active не пуст, обновить кэш активных офферов
      if(!empty($trades_ex_active) || count($trades_ex_active) != 0) {
    ...
    DB::commit(); } catch(\Exception $e) {
        $errortext = 'Invoking of command C8_active_offers_tracking from M-package M16 have ended on line "'.$e->getLine().'" on file "'.$e->getFile().'" with error: '.$e->getMessage();
        DB::rollback();
        Log::info($errortext);
        write2log($errortext, ['M16', 'C8_active_offers_tracking']);
        return [
          "status"  => -2,
          "data"    => [
            "errortext" => $errortext,
