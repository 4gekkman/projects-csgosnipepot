  ...
  public function handle()
  {
    ...
    $params = call_user_func(function() USE ($restype) { try {
      ...
      if($restype == 'm') {
        ...
        $params['github'] = $this->choice('[NOT REQUIRED] Do you want to create a new repo on github for the pack, make in packs folder a local repo, and turn on autopush to github every 3 hours?', ['no', 'yes'], 1);
      ...
      if($restype == 'l') {
        ...
        // 2] Спросить у пользователя, какое имя задать новому L-пакету
        $params['name'] = $this->ask("[NOT REQUIRED] Type name for the new L-package in english", 0);
    ...
    //------------------------------------------------------//
    if(!Cache::has('m1:parse_app:invoking'))
