...
// Консольная команда //
//--------------------//
class T8_pause extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm9:pause';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Pause Play all games by turning on off ticks';
  ...
  public function handle()
  {
    ...
    // 1. Выполнить команду
    $result = runcommand('\M9\Commands\C34_pause', $this->argument());
    ...
    // 3. В случае успеха, сообщить текущее состояние игры

      // 3.1. Получить кэш
      $cache = Cache::get('lottery:pause');

      // 3.2. Если игра не стоит на паузе
      if(empty($cache) || $cache != 1)
        $this->info("Игра снята с паузы");

      // 3.3. Если егра стоит на паузе, снять
      if(!empty($cache) && $cache == 1)
        $this->info("Игра поставлена на паузу");
