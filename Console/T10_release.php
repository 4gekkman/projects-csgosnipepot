...
// Консольная команда //
//--------------------//
class T10_release extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm1:release';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Create new release of specified package';
  ...
  public function handle()
  {
     ...
     * Оглавление
     *
     *  1. Запросить у пользователя параметры для команды
     *  2. Выполнить команду
    ...
    // 1. Запросить у пользователя параметры для команды
      ...
      // 1.1. Подготовить переменную
      $params = [];

      // 1.2. Спросить, для какого пакета создать релиз

        // 1] Получить инфу обо всех пакетах
        $packages = \M1\Models\MD2_packages::query()->pluck('aboutpack', 'id_inner');
        $packages = $packages->map(function($item, $key){
          if($item != "\"\"")
            return '-' . \M1\Models\MD2_packages::where('id_inner','=',$key)->first()->lastversion . '- ' .json_decode($item,true)['EN']['description'];
          else
            return $item;
        });

        // 2] Спросить
        $params['packid'] = $this->choice('For which package do you want to create new release?', $packages->toArray());

      // 1.3. Выяснить тип нового релиза: patch / minor / major
      $params['type'] = $this->choice('[NOT REQUIRED] Choose type of the new release', ["1"=>"patch","2"=>"minor","3"=>"major"], "1");

      // 1.4. Спросить, какую версию присвоить пакету
      $params['version'] = $this->ask("[NOT REQUIRED] Type version for the new release", 0);

    // 2. Выполнить команду
    $result = runcommand('\M1\Commands\C32_release', $params);
