...
// Консольная команда //
//--------------------//
class T14_get_final_items_prices extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm8:get_final_items_prices {items}';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Accepts list of market names of items and returns prices for them';
  ...
  public function handle()
  {
     ...
     * Оглавление
     *
     *  1. Провести валидацию аргумента items
     *  2. Разобрать строку items на массив по символу ','
     *  3. Выполнить команду, передав ей items
     *  4. В случае неудачи, вывести текст ошибки
     *  5. В случае успеха, вывести соотв.сообщение
     *
     */

    // 1. Провести валидацию аргумента items
    $validator = r4_validate($this->argument(), [
      "items"           => ["required", "string"]
    ]); if($validator['status'] == -1) {
      $this->info('Error: '.$validator['data']);
      return;
    }


    // 2. Разобрать строку items на массив по символу ','
    $items = explode(',', $this->argument('items'));


    // 3. Выполнить команду, передав ей items
    $result = runcommand('\M8\Commands\C17_get_final_items_prices', ['items'=>$items]);
