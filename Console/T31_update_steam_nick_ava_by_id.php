...
// Консольная команда //
//--------------------//
class T31_update_steam_nick_ava_by_id extends Command
{
  ...
  // - TODO: настроить шаблон консольной команды
    ...
    protected $signature = 'm5:update_steam_nick_ava_by_id {id?} {id_steam?} {besides_groups?}';

  //-----------------------------//
  // 2. Описание artisan-команды //
  //-----------------------------//

    protected $description = 'Update steam nick and avatar of specified user by steamid';
  ...
  public function handle()
  {
    ...
    // 1. Если аргумент besides_groups присутствует
    // - Заменить строку с разделителями ',', на массив.
    if($this->hasArgument('besides_groups'))
      $besides_groups = $this->argument('besides_groups');
    if(!empty($besides_groups))
      $besides_groups = explode(',', $besides_groups);
    else
      $besides_groups = [];
    ...
    $result = runcommand('\M5\Commands\C75_update_steam_nick_ava_by_id', [
      "id"             => $this->argument('id'),
      "id_steam"       => $this->argument('id_steam'),
      "besides_groups" => $besides_groups,
    ]);
