...
```
  ...
  # Аутентификация: описание процесса распознавания аутентифицированных пользователей
    - Распознавание производится в BeforeMiddleware пакета M5.
    - Вот схема этого процесса наглядно:
      ...
      1) Производится поиск валидного аутентиф.кэша в сессии
        - Если находится, то запрос пропускается дальше.

      2) Производится поиск валидной аутентиф.куки auth, извлекается значение
        - В нём для нас главное, это ID пользователя и ID аутентиф.записи в соотв.таблице.
        - Если находится, то:

          2.1) Производится поиск валидной аутентиф.записи
            - Для этого используют пару извлёчённых ID.
            - Если таковая не найдена

         создаётся аутентиф.кэш в сессии, и запрос пропускается дальше.
