...
```
  ...
  --- Оглавление
    ...
    # Аутентификация: аутентификационный кэш
  ...
  # Аутентификация: аутентификационный кэш
    ...
    - Поэтому, для каждой записи в табл.аутентификаций делается запись и в кэш.
    - Ей задаётся время жизни, оставшееся до конца времени жизни аутентиф.записи в таблице.
    - Её ключ содержит в себе ID пользователя и ID аутениф.записи из таблицы.
    - Эти же данные присутствуют в зашифрованной аутентификационной куке.
    - Ключ строится по шаблону: "auth_cache_" . <user id> . "_" . <auth id>
    - В качестве значения записывается не зашифрованная аутентификационная json-строка.

  # Аутентификация: блокировка пользователя
    - Что происходит при блокировке пользователя:

      1) Удаляется весьаутентификационный кэш пользователя.
  ...
  # Верификация
  # Верификация: очистка верификационных таблиц от старых кодов по расписанию
  ...
  # Аутентификация: как работает
    ...
    • Что происходит при аутентификации
      1) Формируется и шифруется json-строка с моделью пользователя.
      2) В аутениф.таблице создаётся запись, и связывается с этим польз-ем.
        - Эта запись будет валидна N часов (указано в конфиге).
      3) Строка из 1 записывается в куку auth браузера пользователя.
        - Эта кука будет валидна N часов (указано в конфиге).
      4) К кэше создаётся запись о том, что пользователь аутентифицирован.
        - Запись самоуничтожается через N минут после создания (указано в конфиге).

    • Что происходит при каждом заходе пользователя на сайт

      ▪ Если у пользователя нет валидной аутентификационной куки

      ▪ Если у пользователя есть валидная аутентификационная кука



      - При аутентификации json с моделью пользователя шифруется в строку.
      - Она записывается в аутентиф.куку пользователя и в базу данных.
      - При каждом заходе из куки пользователя извлекается эта самая строка.
      - По I

    • Что происходит при блокировке пользователя


    1) Таблица аутентификаций
      -
