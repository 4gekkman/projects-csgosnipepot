      ...
      /project                              |
        ...
        /vendor                             |
          /4gekkman                         |
            ...
            /D<id1>                         | // Здесь
            /D<id2>                         | //  речь о
              bower.json                    | //   том, что все
            /L<id1>                         | //    D,L,W-пакеты
            /L<id2>                         | //     обладают файлом
              bower.json                    | //      bower.json с фронтенд
            /W<id1>                         | //       зависимостями соотв.пакета.
            /W<id2>                         | //        Из них то и собирается итоговый
              bower.json                    | //         файл <проект>/frontend/bower.json
...
Вопрос:
- Как перезапускать gulp.watch / browsersync при изменении paths4gulpwatch.json / paths4browsersync.json?




  • Почему нельзя было полность разместить СУФ в M1

    ▪ Почему нельзя разместить СУФ в composer-пакете M1
      - Потому что многие файлы в СУФ со временем меняются.
      - А при обновлении composer-пакета изменения будут утеряны.

    ▪ Почему нежелательно размещать СУФ в каталоге project с laravel-проектом
      - Потому что в нём д.б. лишь laravel-проект да пакеты к нему.
      - Если мы начнём хранить там какие-то доп.папки, можно в будущем запутаться.
      - И в конце концов забыть, что нам туда надо добавить, чтобы всё работало.
      - А так всё ясно, есть laravel, есть composer-пакеты вендора 4gekkman, ничего лишнего.

    ▪ Лучший вариант для размещения управления, в каталоге на уровне проекта
      - На уровень выше папки project с laravel-проектом.
      - То есть там, где хранятся файлы по управлению docker-проектом.
      - Все файлы СУФ разместить там в отдельном каталоге frontend.
