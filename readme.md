...
```
  ...
  > Адрес репозитория модуля М1 на github
...
Оглавление
  ...
  db_trunc        | m1:db_trunc           | Полностью очистить БД M-пакета с [id]
  db_rubout       | m1:db_rubout          | Удалить из СУБД базы всех отсутствующих M-пакетов
...
//------------------------//
// db_trunc | m1:db_trunc //
//------------------------//

  # Что делает
    ● Полностью очистить БД M-пакета с [id].

  # Аргументы
    - [id] - базу данных пакета с каким ID очистить.


//--------------------------//
// db_rubout | m1:db_rubout //
//--------------------------//

  # Что делает
    ● Удалить из СУБД базы всех отсутствующих модулей.

  # Аргументы
    - Допустим, удалили пакет с ID из composer.json проекта.
    - После этого сделали composer update, пакет исчез из vendors.
    - Но база данных пакета при этом не удаляется.
    - Это сделано специально, чтобы избежать случайной потери данных.
    - Удалить базы таких "отсутствующих" пакетов можно этой командой.
