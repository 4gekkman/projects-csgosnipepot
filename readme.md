...
//------------------------------------//
  ...
  • Автосохранение изменений в пакетах в GitAutoPushScript
    - В пункте 9 сабжа каждые 3 часа данные из всех указанных лок.реп-ев заливаются на github.
    - Там д.б. указаных все LWR- и MD-пакеты серии 10k+, всех проектов.
    - Соответственно, должны учитываться еще и имена проектов.
    ...
    - Если пакет был изначально создан в этом проекте, он автоматом добавится GitAutoPushScript.
    - Клонировать же пакет с github в проект надо спец.командой, она и данные в GitAutoPushScript добавит.

  • Схема деплоя и обновления
    - Проект клон ируется на тестовый сервер. Устанавливаются все модули. Идёт проверка.
    - Если всё OK, процесс повторяется на главном сервере.
    - Обновления сначала тоже тестируются на тестовом, а потом подтягиваются на основной.
    - При обновлении, на до обновлять как сам проект, так и всем модули.
    - Для обновления лучше всего использовать специальный скрипт update на сервере (см.matrix).
