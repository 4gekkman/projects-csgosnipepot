...
```
  ...
  --- Оглавление
    ...
    # Верификация: попытка зарегистрироваться с присутствующим, но не вериф.email
  ...
  # Аутентификация: описание процесса распознавания аутентифицированных пользователей
    ...
    1) Производится поиск валидной аутентиф.куки auth, извлекается значение
      - В нём для нас главное, это ID пользователя и ID аутентиф.записи в соотв.таблице.

    2) Производится поиск валидного аутентиф.кэша
      - Для этого используют пару извлёчённых в 1 ID.
      - Если таковой найден:

        -------> то пользователь пропускается в приложение

    3) Производится поиск валидной аутентиф.записи
      - Для этого используют пару извлёчённых в 1 ID.
      - Если таковая найдена:

        -------> то пользователь пропускается в приложение

    4) Пользователь не пропускается в приложение
