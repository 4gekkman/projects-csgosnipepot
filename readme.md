...
```
  ...
  --- Оглавление
    ...
    # Авторизация: проверка exec-прав в runcommand
    # Авторизация: проверка access-прав в C66_authorize_access в контроллерах DLW-пакетов

    # Защита от брутфорса: как работает
    # Защита от брутфорса: защита аутентификации и механизма смены пароля
  ...
  # Пароль: механизм восстановления пароля через email
    - Пользователь указывает email и запрашивает восстановление.
    - Генерируется и записывается в таблицу аутентиф.кодов случайный цифровой код.
    - Длина этого кода регулируется параметром "auth_code_length" в конфиге.
    - Время жизни этого кода регулируется параметром "email_auth_code_lifetime_min" в конфиге.
    - На указанные email отправляется этот сгенерированный код.
    - Пользователю предлагается ввести этот код в течении указанного времени, а также новый пароль.
    - Если введён верный код, то производится смена пароля.

  # Пароль: механизм восстановления пароля через sms
    - Аналогично механизму с email, только через sms.
  ...
  # Аутентификация: описание процесса распознавания аутентифицированных пользователей
    - Распознавание производится в AfterMiddleware пакета M5, командой C56_meet.
  ...
  # Аутентификация: где происходит распознавание, как данные передаются клиенту
    ...
    • Аутентификация реализуется в AfterMiddleware пакета M5
  ...
  # Авторизация: проверка exec-прав в runcommand
    ...
    • Хелпер lib_current_user_id
      - Runcommand принимает 3-им аргументом ID пользователя, который запускает команду.
      - Если этот аргумент == 0, то проверка exec-прав пользователя не производится.
      - При применении хелпера, конечно, можно и указать этот ID вручную.
      - Но в 99% случаев (авто-обрабатываемые ajax-запросы) ID вычисляется с помощью
        другого хелпера: lib_current_user_id
      - Он достаёт из сессии значение по ключу "auth_cache", смотрит json-ли это,
        если да расшифровывает в массив, ищет там ['user']['id'], и если это
        целое число от 1 и выше, то берёт его в качестве ID.
      - Если же ID найти не удаётся, значит пользователь не аутентифицирован,
        и у него нет никаких прав. Хелпер возвращает -1, и runcommand, получив
        это значение возвращает ответ со статусом 1 (доступ запрещён):
    ...
    • Хелпер runcommand, процесс exec-авторизации
      - Проверка exec-прав пользователя производится в хелпере "runcommand" из пакета R1.
      - Все команды в системе запускаются всегда только через этот хелпер.
      - Сначала runcommand проверяет ID пользователя. Если это -1 или не 0/целое полож.число,
        то доступ запрещён (см.выше).
        Если это 0, то проверка прав не производится (режим бога). Если это целое число от 1
        и выше, то происходит процесс проверки прав.
      - Runcommand ищет в сессии значение по ключу "authorize_exec". В нём должен лежать массив
        строк с полностью квалифицированными путями к командам, которые пользователь имеет право
        выполнять.
      - Если authorize_exec не пусто и валидно, то runcommand ищет значение из своего аргумента
        $command в этом массиве. Если не находит, возвращает ответ со статусом 1 (доступ запрещён).
        Если находит, допускает выполнение команды.
      - Если authorize_exec пусто или не валидно, то runcommand извлекает из базы данных массив
        полностью квалиф.имён всех команд, которые этот пользователь имеет право выполнять.
        И ищет значение из своего аргумента $command в этом массиве. Если не находит, возвращает
        ответ со статусом 1 (доступ запрещён). Если находит, допускает выполнение команды.

    • Вкл / Выкл exec-авторизацию в конфиге
      - За это отвечает параметр "authorize_exec_ison" в конфиге.

  # Авторизация: проверка access-прав в C66_authorize_access в контроллерах DLW-пакетов

    • Команда C66_authorize_access
      - Эта команда в качестве аргумента принимает ID (inner) DLW-документа.
      - Весь процесс авторизации access-прав производится в этой команде.
      - Авторизация производится во всех контроллерах DLW-пакетов, для get- и post-запросов,
        в начале методов getIndex и postIndex.
      - Сначала authorize_access проверяет ID пользователя. Если это -1 или не целое полож.число,
        то доступ запрещён, и команда возвращает статус -1.
        Если это целое число от 1 и выше, то происходит процесс проверки прав.
      - Authorize_access ищет в сессии значение по ключу "authorize_access". В нём должен лежать массив
        строк с id_inner DLW-пакетов, к документам которых пользователь имеет право доступа.
      - Если массив authorize_access не пуст и валиден, то команда ищет id_inner (из своего аргумента)
        в этом массиве. Если не находит, возвращает ответ со статусом -1 (доступ запрещён).
        Если находит, возвращает ответ со статусом 0 (доступ разрешён).
      - Если массив authorize_exec пуст или не валиден, то команда из базы данных массив
        ID DLW-пакетов, к которым этот пользователь имеет право доступа.
        И ищет id_inner (из своего аргумента) в этом массиве. Если не находит, возвращает
        ответ со статусом -1 (доступ запрещён).
        Если находит, возвращает ответ со статусом 0 (доступ разрешён).

    • Что контроллер DLW-пакета возвращает в случае отказа в доступе
      - Если команда C66_authorize_access возвращает статус 0, значит доступ к этому
        интерфейсу для этого пользователя разрешён, и мы пропускаем запрос дальше (ничего не делая с ним).
      - Если же команда C66_authorize_access возвращает статус -1 (доступ запрещён), то
        мы возвращаем клиенту следующее:
  ...
  # Защита от брутфорса: как работает
    ...
    - Защиту можно вкл./выкл. с помощью параметра "bruteforce_protection_ison" в конфиге.
    - В каких случаях срабатывает:

      1) При неудачной попытке аутентификации.
      2) При неудачной попытке верификации.
      3) При неудачной попытке смены пароля.

    - Принцип работы:

      1) При неудачной попытке из сессии по ключу "broteforce_attempts" . "." . <ID пользователя> извлекается значение.
      2) Если там пусто или не число, но по этому же ключу записывается значение 1.
         Если там число, то записывается это число + 1.
         При записи назначается время жизни "bruteforce_protection_counter_lifetime" минут (из параметра в конфиге).
      3) В механизмы аутентиф., вериф., смены пароля встроена проверка этого счётчика.
         Если величина числа в счётчике больше "bruteforce_protection_threshold", то операция блокируется.

    - Как применять на практике:

      1) Сначала получаем пользователя, для которого будем проводить проверку.
      2) Затем применяем команду C67_bruteforce_check, передав ей id пользователя.
        - Команда проверяет счётчик брутфорса для этого пользователя.
        - Если предел превышен, команда вернёт статус -1. А если нет, то 0.
      3) В обработке исключений применяем команду C68_bruteforce_add
        - Но только в том случае, если пользователь в 1 найден, передавая её его id.
        - Эта команда просто прибавляет +1 к счётчику брутфорса для этого пользователя в кэше.

  # Защита от брутфорса: защита аутентификации и механизма смены пароля
    - Одновременно используется один и тот же механизм защиты.
    - С общим счётчиком.
...
```
  [!!! TODO: Тут описаны не все команды, надо дополнить !!!]
