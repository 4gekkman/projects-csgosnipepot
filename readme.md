...
--------------------------------------
  ...
  • Нельзя подключать bower-пакеты latest-версий
    - Потому что они со временем могут изменяться.
    - Могут менятсья файлы и их расположение.
    - В результате чего фронтенд может ломаться при обновлении.
    - А надо в проекте использовать лишь конкретные версии.

  • Mains и gulp-tasks для bower-пакетов надо хранить в R-пакете
    - Который является зависимостью пакета M1.
    - Потому что многие DLW-пакеты могут использовать один и тот же bower-пакет.
    - Глупо было бы постоянно прописывать одни и те же mains и gulp-tasks.
    - Надо просто в R-пакете сделать специальную папочку.
    - А в неё могут лежать другие папочки, как раз с mains и gulp-tasks для задач.
    - Имена этих папочек будут содержать: имя пакета + semver-ограничение.

  • Что, если для какого-либо bower-пакета нет "папочки" в R-пакете?
    - Будет специальная задача, которая будет проверять это.
    - Если обнаружется установленный bower-пакет без "папочки", будет сообщать.
    - Никакие ресурсы такого пакета не попадут в public или в blade-документ.

  • Исходники и результирующие для bower-пакетов не требуется отслеживать
    - Нет необходимости запускать gulp.watch или bs.watch для этого.
    - Ведь bower-пакеты не изменяются, они уже ранее были разработаны.
    - При общем обновлении bower-пакетов нужно лишь разово выполнять компиляцию/обработку.
    - Этим, собственно говоря, и занимаются gulp-tasks bower-пакетов в R-пакете.
    - Надо написать команду, которая обходит все установленные bower-пакеты,
      и просто выполняет задачи для каждого пакета с заданным именем.
    - В результате чего ресурсы пакетов компилируются/обрабатываются и попадают в public-проекта.

  • Применяем gulp.watch/bs.watch только для разрабатываемых DLW-пакетов
    - Нет смысла делать watch'и, отслеживающий вообще все пакеты.
    - Поскольку NFS с виндой не работает, и usePooling тогда будет очень медленным.
    - Вместо этого есть смысл делать следующее:
      ▪ Поместить в корень разрабатываемого пакета файл gulpfile.js.
      ▪ В нём разместить gulp.watch, который:
        ▪ Наблюдает за всей папкой с css/js исходниками.
        ▪ При изменениях компилирует их (+assets) в public проекта.
      ▪ В нём разместить bs.watch, который:
        ▪ Наблюдает за всей папкой с результатами DLW-пакета в public проекта.
        ▪ Создаёт мини-веб-сервер с ip и портом, позволяющий верстать с livereload.
    - Если мы хотим сейчас вести разработку этого DLW-пакета, мы делаем следующее:
      ▪ Переходим в корень этого DLW-пакета.
      ▪ Выполняем с помощью gulp соответствующую задачу, запускающую демонов.
    - Что мы делаем в рамках к.команды m1:suf
      ▪ Единоразово компилируем и публикуем ресурсы и assets DLW-пакета.

  • Mains DLW-пакетов храним в bower.json этих пакетов
    - А, скажем, порядок конкатенации указываем непосредственно в gulpfile.js.
