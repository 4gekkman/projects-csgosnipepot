...
## Оглавление
  ...
  - [Прикладные инструкции по работе с СУБ и СУФ](#link5)
	...
	- [Стратегия продажи и сервиса проектов v1](#link9)
	- [Стратегия продажи и сервиса проектов v2](#link10)
...
# Что умеет СУБ
  ...
  • Синхронизировать модели указанного пакета с его БД (C36_workbench_sync)
    ...
    - В каждой модели она прописывает все связи этой мадели, в т.ч. транспакетные.
    - А также проставляет withPivot для каждой связи (столбцы pivot-таблиц без префикса "id_").
...
## Стратегия продажи и сервиса проектов v1 <a id="link9"></a>
```
---------------------------------------
Подоглавление:

  Стратегия работы с пакетами
  ---------------------------

    • Для всех проектов стараемся использовать одни и те же MR-пакеты
    • Для каждого проекта, где будут отличия в интерфейсах (таких 99%), создаём свои DLW-пакеты
    • Все пакеты должны иметь свои репозитории на github, и быть приватными
    • Все исходники пакетов должны находиться в проекте devapp
    • Все пакеты должны автоматически сохраняться на github, минимум ежедневно
    • Раз в 3 дня должна автоматически делаться копия всех проектов и заливаться в google drive

  Систематизация проектов и пакетов
  ---------------------------------

    • Отдельные M-пакет и D-пакет для этой задачи
    • Древообразная структура зависимостей пакетов в проектах
    • Инфа о принадлежности к проекту и версиях ключевых технологий в composer.json пакетов
    • Команда parseapp из M1 должна собирать и передавать информацию из composer.json пакетов

  Процесс разработки от момента заказа до релизной версии
  -------------------------------------------------------

    • Вся разработка ведётся внутри проекта devapp от начала до конца
    • Каждому заказчику в системе управления пользователями настраиваются доступы
    • Заказчик имеет доступ соотв.интерфейсам через реальный IP и указанный порт
    • Переписка с заказчиком осуществляется через telegram
    • Заказчик видит весь процесс разработки, и текущую стадию, в wunderlist

  Процесс подготовки проекта к деплою на сервер заказчика
  -------------------------------------------------------

    • Каталог с проектом devapp копируется в projects и переименовывается именем проекта
    • Надо убедиться, что у всех M-пакетов правильные установочные sql-файлы.
    • Производится подготовка проекта:
      ▪ Со всеми следами удаляются все пакеты вендора 4gekkman
      ▪ Все зависимости этого проекта прописываются в composer.json
      ▪ С помощью composer update устанавливаются все зависимости проекта
      ▪ С помощью команды m1:run производится приведение проекта в актуальное состояние
      ▪ Производится настройка всех конфигов проекта
      ▪ Проект запускается и тестируется на компьютере разработчика
      ▪ Если заказчик не заплатил за сервисное обслуживание, ключи доступа к github изымаются

  Процесс деплоя проекта на сервер заказчика
  ------------------------------------------

    • Для деплоя заказчику понадобится полноценный VPS-сервер
    • Папка с проектом заливается на сервер заказчика с помощью rsync
    • Проект запускается на сервере заказчика

  Процесс сервисного обслуживания проданных проектов
  --------------------------------------------------

    • Все правки выполняются в проекте devapp
    • После внесения правок, надо делать бамп версий исправленных пакетов на github
    • Затем два варианта, залив через доступ к серверу, либо через github
      ▪ Либо получить доступ к серверу заказчика, и вручную залить правки
      ▪ Либо выполнить composer update на сервере, где установлен пакет (прописав ключи к github)
...
//-------------------------------//
  ...
  • Для всех проектов стараемся использовать одни и те же MR-пакеты
    - Потому что в большинстве случаев в них от проекта к проекту ничего не надо менять.
    - Если изменение небольшое, то можно просто дополнить существующий проект.
    - Если же изменения серьёзные, тогда можно, конечно, создать новый MR-пакет, форкнув старый (может, 1% всех случаев).

  • Для каждого проекта, где будут отличия в интерфейсах (таких 99%), создаём свои DLW-пакеты
    - В большинстве проектов (наверное, 99%) будут отличия в интерфейсах (публичных, админки и т.д.).
    - Поэтому, для новых проектов, лучше сразу создавать отдельные DLW-пакеты.

  • Все пакеты должны иметь свои репозитории на github, и быть приватными
    - Абсолютно все пакеты должны иметь свои репозитории на github.
    - Это необходимо, преимущественно, из соображений резервного копирования, а также постпродажного обслуживания.

  • Все исходники пакетов должны находиться в проекте devapp
    - Это удобно, когда все исходники пакетов находятся в одном месте.
    - И всю разработку можно вести из одного проекта.

  • Все пакеты должны автоматически сохраняться на github, минимум ежедневно
    - Это необходимо из соображений резервного копирования данных.
    - У меня подобное сохранение на данный момент (2016-07-09) настроено на каждые 3 часа.

  • Раз в 3 дня должна автоматически делаться копия всех проектов и заливаться в google drive
    - Кроме пакетов в проектах есть ещё много всего ценного.
    - Поэтому, необходимо периодически сохранять всю папку WebDev куда-нибудь в облако.
    - У меня подобное сохранение настроено производитсья раз 3 дня, сохраняя архив с папкой в Google Drive.
...
//-------------------------------------//
  ...
  • Отдельные M-пакет и D-пакет для этой задачи
    - Для систематизации проектов и пакетов лучше создать отдельный M-пакет.
    - У него должна быть база с проектами, пакетами, древообразными зависимостями и так далее.
    - Для доступа ко всей этой информации надо будет также создать отдельный D-пакет.

  • Древообразная структура зависимостей пакетов в проектах
    - Между пакетами существует древообразная система зависимостей.
    - Какой пакет зависит от какого, описано в composer.json (для бэкенд-зависимостей) и bower.json (для фронтенд-зависимостей).

  • Инфа о принадлежности к проекту и версиях ключевых технологий в composer.json пакетов
    - В composer.json каждого бэкенд-пакета (MDLWR) есть информация о том, к каким проектам он принадлежит.
    - Также есть информацию о том, с какой версией Laravel этот пакет должен работать.
    - Всю эту информацию, а также информацию из bower.json, будет утилизировать пакет по систематизации проектов.

  • Команда parseapp из M1 должна собирать и передавать информацию из composer.json пакетов
    - Эта команда как раз будет собирать всю необходимую информацию для систематизации проектов.
    - И передавать её с помощью события M-пакету по систематизации проектов.
...
//-----------------------------------------------------------//
  ...
  • Вся разработка ведётся внутри проекта devapp от начала до конца
    - Это довольно таки удобно, вести разработку в одном месте.
    - Все новые пакеты создаются в проекте devapp.

  • Каждому заказчику в системе управления пользователями настраиваются доступы
    - Когда проект devapp запущен на компьютере разработчика, заказчик сможет зайти.
    - Но чтобы он видел только свои интерфейсы, система ограничения доступов должна быть включена.
    - Вход только по логину и паролю через эту систему.
    - Каждый заказчик получает аккаунт в системе, с доступами только к его интерфейсам.
    - Все доступы только напросмотр, доступы к действиям по запросам.

  • Заказчик имеет доступ соотв.интерфейсам через реальный IP и указанный порт
    - Доступ к проекту заказчик имеет через реальный IP и указанный порт.
    - Если реального IP нет, то доступ к проекту на компьютере разработчика обеспечить не удастся.
    - Пример такой ситуации, когда разработчик где-то пушетествует, где у него нет реального IP.

  • Переписка с заказчиком осуществляется через telegram
    - Это удобно, поскольку есть как десктопные, так и мобильные версии этого месенджера.
    - В нём можно создавать группы, и все практически всегда на связи.

  • Заказчик видит весь процесс разработки, и текущую стадию, в wunderlist
    - В wunderlist можно проставить все чекпоинты проекта.
    - И показывать заказчику, где мы находимся в настоящий момент.
...
//-----------------------------------------------------------//
  ...
  • Каталог с проектом devapp копируется в projects и переименовывается именем проекта
    - Просто вот так берётся, и целиком копируется.

  • Надо убедиться, что у всех M-пакетов правильные установочные sql-файлы.
    - Чтобы при установке пакетов, как зависимостей, все базы установились правильно.
    - При установке баз, если такая база существует, не устанавливать её.

  • Производится подготовка проекта

    ▪ Со всеми следами удаляются все пакеты вендора 4gekkman
      - Сама папка вендора 4gekkman.
      - Все конфиги из config.
      - Все пространства имён из composer.json.
      - Все задачи из планировщика задач (app/Console/Kernel.php).
      - Все провайдеры из app.php (config/app.php -> providers).
      - И так далее...

    ▪ Все зависимости этого проекта прописываются в composer.json
      - Можно добавить сюда лишь корневые зависимости проекта 1-го уровня.
      - Какие из них являются коревыми, см.в интерфейсе по систематизации проектов.
      - Для установки может потребоваться ключ к github на чтение приватных пакетов.

    ▪ С помощью composer update устанавливаются все зависимости проекта
      - Все пакеты и их зависимости будут установлены.
      - Все отсутствующие базы данных будут установлены.

    ▪ С помощью команды m1:run производится приведение проекта в актуальное состояние
      - Это позволит выполнить начальную настройку проекта.
      - Прописать провайдеры, настроить планировщики и так далее.
      - Возможно, потребуется выполнить m1:run пару-тройку раз.

    ▪ Производится настройка всех конфигов проекта
      - Надо выполнить настройку всех конфигов проекта.
      - После выполнения m1:run конфиги пакетов пропишутся в config.
      - И вот там надо все параметры и настроить. Можно просто тупо скопировать из devapp.

    ▪ Проект запускается и тестируется на компьютере разработчика
      - Выполняется . app up и новый проект запускается.
      - Проверяется работоспособность всех систем, наличие ошибок в логе.

    ▪ Если заказчик не заплатил за сервисное обслуживание, ключи доступа к github изымаются
      - То есть, заказчик не сможет сделать composer update для доступа к приватным репозиториям.
      - И не сможет обновлять свои пакеты до новых версий.
...
//----------------------------------------------//
  ...
  • Для деплоя заказчику понадобится полноценный VPS-сервер
    - То есть, это должен быть сервер с полным доступом ко всей системе.

  • Папка с проектом заливается на сервер заказчика с помощью rsync
    - Только через rsync можно залить проект на сервер так, чтобы по дороге ничего не "потерялось".

  • Проект запускается на сервере заказчика
    - Проект уже был протестирован и отлажен ранее на компьютере разработчика.
    - Поэтому, осталось запустить его на сервере заказчика (. app up), и готово.
...
//------------------------------------------------------//
  ...
  • Все правки выполняются в проекте devapp
    - Необходимые правки вносятся в соответствующие пакеты.

  • После внесения правок, надо делать бамп версий исправленных пакетов на github
    - Надо сначала выполнить push.
    - Потом сделать бамп версий изменённых пакетов (есть специальная команда в M1).

  • Затем два варианта, залив через доступ к серверу, либо через github

    ▪ Либо получить доступ к серверу заказчика, и вручную залить правки
      - Этот вариант более предпочтителен.
      - Поскольку могут понадобиться правки вне пакетов.

    ▪ Либо выполнить composer update на сервере, где установлен пакет (прописав ключи к github)
      - Этот вариант тоже имеет право на жизнь.
      - Когда все правки внутри пакетов, и не затрагивают БД, вполне себе норм.


```

## Стратегия продажи и сервиса проектов v2 <a id="link10"></a>
```
---------------------------------------
Подоглавление:
  ...
  # Полная картина в мелком масштабе
    ...
    ▪ Одновременная работа над 1-им MDLWR-пакетом в >= 2 проектах
    ▪ Создание новых пакетов командой m1:new
    ▪ Автосохранение изменений в пакетах в GitAutoPushScript
    ▪ Схема деплоя и обновления
...
//------------------------------------//
  ...
  • MDLWR-пакеты
    - Есть MDLWR-пакеты. Есть MD-пакеты серии 10k+, а есть серии 1-9999.
    - Удалённые репозитории на github имеют LWR-пакеты, и MD-пакеты серии 10k+.
    - Все удалённые репозитории принадлежат одному пользователю 4gekkman на github.

  • Проекты и их окружение
    - Каждый проект лежит в отдельной папке, и имеет удалённый репозиторий.
    - Проект состоит из окружения и MDLWR-пакетов, оформленных, как модули проекта в .gitmodules.
    - То, что лежит в репозитории проекта, это его окружение, и ссылки на модули.
    - У каждого проекта уникальное окружение => нельзя все модули разрабатывать в одном окружении devapp.

  • Одновременная работа над 1-им MDLWR-пакетом в >= 2 проектах
    - Рассмотрим 2 проекта, P1 и P2, и пакет M15, который есть в P1, но нет в P2.
    - Сначала клонируем M15 в P2 с github, и настраиваем его работу.
    - Затем, в P1 и P2 отдельно, вносим всякие изменения в M15.
    - Заливаем изменения на github по схеме commit -> pull -> push.
    - Получается та же схема, когда над 1-им репозиторием работают N программистов.
    - При этом, пакет должен оставаться совместимым со всеми работающими проектами, где он используется.

  • Создание новых пакетов командой m1:new
    - В любом проекте можно создавать новые пакеты командой m1:new.
    - Причём, MD-пакеты серии 1-9999 можно создавать с любыми ID.
    - А вот LWR- и MD-пакеты серии 10k+ только с теми ID, с которыми нет оных на github/4gekkman.

  • Автосохранение изменений в пакетах скриптом autosave2github.ps1
    - У каждого проекта есть свой индивидуальный скрипт autosave2github.ps1.
    - На Linux будет аналогичный скрипт, только для bash.
    - Этот скрипт должен выполняться местным cron'ом каждые N часов.
    - Он автосохраняет репо проекта, и всех его LWR- и MD-пакетов серии 10k+.
    - Заливы должны происходить по схеме commit -> pull -> push.
    - Периодически надо выполнять команду вручную и палить, нет ли конфликтов.
    - Если пакет был изначально создан в этом проекте, он автоматом добавится в скрипт.
    - Клонировать же пакет с github в проект надо спец.командой, она и данные в скрипт также добавит.

  • Схема деплоя и обновления

    ▪ Подготовка
      - Создаём отдельный аккаунт проекта на github.
      - Создаем oauth-токен с правом repo (чтение/запись репозиториев) под него.
      - Вписываем в конфиг M1 проекта этот токен, а также имя владельца github-аккаунта проекта.
      - Файл .gitmodules автоматически генерируется при run_light на основе этих данных.
      - Добавляем аккаунт проекта в collaborators необходимых репо у 4gekkman,
        после чего он может делать приватные форки этих приватных репозиториев.
      - Форкаем в проект репы проекта и всех используемых им подмодулей из 4gekkman.

    ▪ Деплой
      - На сервере делаем git clone --recursive форка проекта из его репо.
      - Применяем http-адрес проекта, используем всё тот же токен проекта.
      - Не забываем после этого сделать chmod 777 для папки проекта.

    ▪ Обновление
      - Сначала надо обновить все форки на github-репо проекта (зайти в каждый, кнопка new pull request).
      - Потом применить скрипт update на сервере (как на matrix). Он обновит проект и подмодули.
  ...
  • Мозговой штурм
    ...
    - MDLWR-пакеты.
    - MD-пакеты серии 10k+
    - MD-пакеты серии 1-9999.
    - Репозитории LWR-пакетов и MD-пакетов серии 10k+.
    - Создание новых пакетов командой m1:new.
    - Различия в окружениях разных проектов,
      невозможность разрабатывать все проекты в devapp.
    - GitAutoPushScript.ps1
    - Каждый проект также имеет отдельный репозиторий.
    - В репозитории проекта, репозитории пакетов д.б. зарегистрированы, как модули (в .gitmodules).
    - При заливе на сервер клонируется реп.проекта, затем устанавливаются модули.
    - Для обновления, обновляются отдельно реп.проекта, и отдельно все модули.

    Как работать с одним и тем же пакетом одновремено из разных проектов?
    - Есть два проекта P1 и P2.
    - Оба проекта используют пакет M1, и могут вносить в него правки.
    - Таким образом, пакет M1 рассматривается, как код, над которым трудятся несколько разработчиков.
    - Порядок внесения изменения такой: commit -> pull -> push.
    - Автовнесение изменений для M1 д.б. настроено для обоих проектов.

    В GitAutoPushScript.ps1 надо внести изменения
    - Речь пойдёт о пункте "9. Сохранение моих laravel-пакетов на github".
    - Сейчас он конкретно настрое на работу только с проектом devapp.
    - Надо в этот функционал, а также в соотв. команду M1 внести изменения.
    - Чтобы учитывалось название папки с проектом при добавлении в GitAutoPushScript.
    - И чтобы в GitAutoPushScript также учитывались имена проектов.
    - А также, чтобы формат залива был такой: коммит -> pull -> push.

    Схема деплоя на рабочий сервак
    - Сначала деплой на тестовый, проверка функционала.
    - Потом деплой на рабочий.

    - Ветки master и test делать смысла нет.
    - Все равно функционал расбросан по разным пакетам.
    - А делать для каждого пакета ещё и ветки, ёбнешься.
