 ...
 *
 *  s2. Модель победителей
 *
 *  	s2.1. Объект-контейнер для всех свойств модели
 *  	s2.2. Наблюдаемый массив с победителями
 *  	s2.3. Выбранный победитель
 *  	s2.4. Модель переименования победителей
 *  	s2.5. Модель изменения аватаров победителей
 *  	s2.n.	Индексы и вычисляемые значения
 *
 *  s3. Модель раздела "победить"
 *
 *  	s3.1. Объект-контейнер для всех свойств модели
 *  	s3.2. Наблюдаемый массив с комнатами
...
var ModelProto = { constructor: function(ModelFunctions) {
	...
	// s2.2. Наблюдаемый массив с победителями //
	//-----------------------------------------//
	self.m.s2.winners = ko.mapping.fromJS(server.data.winners);

	//----------------------------//
	// s2.3. Выбранный победитель //
	//----------------------------//
	self.m.s2.choosen = ko.observable(self.m.s2.winners()[0]);
	...
	self.m.s2.rename = {};
		...
		// 1] Победитель, находящийся в режиме переименования
		self.m.s2.rename.winner = ko.observable("");

		// 2] Значение input'а, предназначенного для переименования победителя
		self.m.s2.rename.input = ko.observable("");
	...
	self.m.s2.chava = {};
		...
		// 1] URL изображения, которое надо загрузить
		self.m.s2.chava.inputurl = ko.observable("");

		// 2] Победитель, находящийся в режиме изменения аватара
		self.m.s2.chava.winner = ko.observable("");
	...
	// s3.2. Наблюдаемый массив с комнатами //
	//--------------------------------------//
	self.m.s3.rooms = ko.mapping.fromJS((function(){

		// 1] Добавить каждой комнате доп.свойства
		for(var i=0; i<server.data.rooms.length; i++) {
			server.data.rooms[i].ticket2win = "";
		}

		// 2] Вернуть результаты
		return server.data.rooms;
